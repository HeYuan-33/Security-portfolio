# About me
I am a blockchain security researcher and actively participate in various types of competitions, involving languages such as **Solidity** and **Move**.
# Security-portfolio
## MoveBit
| Project         | Type      | Language  | Finding                     |
|:---------------:|:---------:|:--------:|:---------------------------:|
| xMoney          | Token     | Move     | 1 Medium                    |
| Deeptrade       | DEX       | Move     | 3 Medium, 4 Minor           |
| Meme Strategy   | DeFi      | Solidity | 1 Medium, 3 Minor           |
| Haedal Protocol | Staking   | Move     | 2 Medium, 1 Minor           |
| Goblin          | DeFi      | Move     | 3 Medium, 2 Minor           |
| Solido          | DeFi      | Move     | 2 Major, 1 Medium, 2 Minor  |
| Dexter          | DeFi      | Move     | 1 Minor                     |
| Scallop         | DeFi      | Move     | 1 Major, 3 Minor            |
| Navi Protocol   | DeFi      | Move     | 2 Medium                    |
| Nemo Protocol   | DeFi      | Move     | 2 Medium, 1 Minor           |
| Surge           | Launchpad | Move     | 1 Medium, 1 Minor           |
| Proly           | DeFi      | Move     | 1 Major,1 Medium, 1 Minor   |
| Yeap Finance    | DeFi      | Move     | 2 Major,2 Medium            |
| Umi             | Wallet    | Move     | 1 Medium, 3 Minor           |
| OPENSCI         | DeFi      | Solidity | 1 Major, 3 Minor            |

## Public Audit Competition
[code4rena](https://code4rena.com/@Heyu)
| Project             | Language  | Finding   |
|:------------------:|:--------:|:--------:|
| Virtuals Protocol   | Solidity | 2 High   |

[sherlock](https://audits.sherlock.xyz/watson/hy)
| Project                        | Language  | Finding           |
|:-------------------------------:|:--------:|:----------------:|
| DODO Cross-Chain DEX            | Solidity | 1 Medium          |
| stNXM by EaseDeFi               | Solidity | 1 High, 1 Medium  |
| Monolith Stablecoin Factory     | Solidity | 1 High            |

[hackenproof](https://hackenproof.com/hackers/Yier)
| Project             | Language  | Finding          |
|:------------------:|:--------:|:----------------:|
| DexLyn Tokenomics   | Move     | 1 High, 2 Medium |
| Reliq               | Solidity | 1 Low            |


 
